...
const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      ...
      disabled={disabled}
    >
      {startIcon && <span className="flex items-center leading-none shrink-0">{startIcon}</span>}
      {children}
      {endIcon && <span className="flex items-center leading-none shrink-0">{endIcon}</span>}
    </button>
  );
};
